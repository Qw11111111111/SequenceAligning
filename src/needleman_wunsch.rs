...
impl ScoreMatrix {
    ...
    fn backtrace(&self, seq1: &str, seq2: &str, local: bool) {
        ...
        for start_ in &start {
            ...
            get_next(&self.paths, seq1, seq2, *start_, &mut hit);
        }
    }
}
...
pub fn n_w_align<'a>(seq1: &Record, seq2: &Record, verbose: bool, local: bool) -> Result<'a, ()> {
    ...
    let s1 = &vec_u8_to_str(&seq1.seq);
    let s2 = &vec_u8_to_str(&seq2.seq);
    mat.init(s1, s2);
    mat.fill(s1, s2, &scheme, local);
    if verbose {
        println!("search finished after {:#?}", now.elapsed());
    }
    println!(
        "Alignment between sequences {} and {} found",
        vec_u8_to_str(&seq1.name),
        vec_u8_to_str(&seq2.name)
    );
    mat.backtrace(s1, s2, local);
    Ok(())
}

fn get_next(paths: &PathArrray, seq1: &str, seq2: &str, current: (usize, usize), hit: &mut Hit) {
    if current == (0, 0) {
        println!("\nHit: {}\n", hit);
        return;
    }
    ...
    for p in &paths[current.0][current.1] {
        ...
        get_next(paths, seq1, seq2, next, hit);
        hit.query.pop();
        hit.db.pop();
    }
}
